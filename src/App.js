...
import { addFav, fetchAnother, getFavsFromLocalStorage, removeFav } from "./actions";
  ...
  return (
    <div className="wrapper max-w-xl mx-auto px-4">
      <nav className="flex text-2xl pb-6 pt-8 gap-2 justify-center">
        ...
        <NavLink to="/favs" className="py-3 px-6 " activeClassName="bg-white shadow-sm text-blue-600">
          Favoriler
        </NavLink>
      </nav>
      ...
      <Switch>
        <Route exact path="/">
          ...
          <div className="flex gap-3 justify-end py-3">
            ...
            <button onClick={addToFavs} className="select-none px-4 py-2 bg-blue-700 hover:bg-blue-600 text-white">
              Favorilere ekle
            </button>
          </div>
        </Route>
        ...
        <Route path="/favs">
          <div className="flex flex-col gap-3">
            {favs.length > 0 ? (
              favs.map((item) => <FavItem key={item.key} id={item.key} data={item} removeFav={removeFav} />)
            ) : (
